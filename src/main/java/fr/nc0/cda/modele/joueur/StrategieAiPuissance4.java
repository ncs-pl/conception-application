...
public class StrategieAiPuissance4 implements Strategie {
    ...
    public Choix jouer(Ihm ihm, Plateau plateau, Joueur joueur) {
        ...
        if(actualiserEtatPartie(plateauRotation(RotationPuissance4.HORAIRE, p4), CellulePuissance4.JAUNE)){
            System.out.println("Horaire");
            return new ChoixPuissance4(CoupPuissance4.ROTATION, RotationPuissance4.HORAIRE, 0);
        } else if (actualiserEtatPartie(plateauRotation(RotationPuissance4.ANTI_HORAIRE, p4),CellulePuissance4.JAUNE)) {
            System.out.println("Anti-Horaire");
            return new ChoixPuissance4(CoupPuissance4.ROTATION, RotationPuissance4.ANTI_HORAIRE, 0);
        }

        List<List<Integer>> listCoup = new ArrayList<List<Integer>>();
        for (int i = 0; i < 7; ++i){
            listCoup.add(new ArrayList<Integer>());
        }
        ...
        for (int i = listCoup.size() - 1; i >= 0; --i){
            for (int j = listCoup.get(i).size() - 1; j >= 0; --j) {
                if (!actualiserEtatPartie(plateauRotation(RotationPuissance4.HORAIRE, p4), CellulePuissance4.ROUGE)
                || !actualiserEtatPartie(plateauRotation(RotationPuissance4.ANTI_HORAIRE, p4), CellulePuissance4.ROUGE)){
    ...
    private PlateauPuissance4 plateauRotation(RotationPuissance4 rotation, PlateauPuissance4 plateau)
    {
    ...
    private int comptageLigne(PlateauPuissance4 plateau, CellulePuissance4 joueurCellule, int colonne, int ligne){

        int[] liste = new int[4];

        //horizontal
        liste[0] = 1 + comptageCellule(plateau, joueurCellule, colonne + 1, ligne, new int[]{1,0})
            + comptageCellule(plateau, joueurCellule, colonne - 1, ligne, new int[]{-1,0});

        //vertical
        liste[1] = 1 + comptageCellule(plateau, joueurCellule, colonne, ligne + 1, new int[]{0,1})
            + comptageCellule(plateau, joueurCellule, colonne, ligne - 1, new int[]{0,-1});

        //diag1
        liste[2] = 1 + comptageCellule(plateau, joueurCellule, colonne + 1, ligne + 1, new int[]{1,1})
            + comptageCellule(plateau, joueurCellule, colonne - 1, ligne - 1, new int[]{-1,-1});

        //diag2
        liste[3] = 1 + comptageCellule(plateau, joueurCellule, colonne + 1, ligne - 1, new int[]{1,-1})
            + comptageCellule(plateau, joueurCellule, colonne - 1, ligne + 1, new int[]{-1,1});
        ...
        System.out.println("Couleur : " + joueurCellule.toString() + "Max : " + max + " | Colonne : " + colonne + " | Ligne : " + ligne);
        return max;
    }

    private int comptageCellule(PlateauPuissance4 plateau, CellulePuissance4 joueurCellule, int colonne, int ligne, int[] operation) {
        if (plateau.getCellule(colonne,ligne) == joueurCellule && colonne >= 1 && colonne <= plateau.getLongueur() && ligne >= 1 && ligne <= plateau.getHauteur()){
            return 1 + comptageCellule(plateau, joueurCellule, colonne + operation[0], ligne + operation[1], operation);
        } else {
            return 0;
        }
    }
    ...
    private PlateauPuissance4 makeCopiePlateau(PlateauPuissance4 plateau){
        ...
        PlateauPuissance4 nouveauPlateau = new PlateauPuissance4(plateau.getLongueur(), plateau.getHauteur());
